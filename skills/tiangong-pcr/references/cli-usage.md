...
`list` and `tree` are explicit catalog browsing commands. `list` defaults to 10 records per page and returns pagination metadata in JSON output. Prefer `resolve` when a classification code is available. Use `validate-dataset` for foreground collection package coverage.
